...
- Create a channel for communication with the One Piece Group Bot called TG Rest Channel, and add
  the TG Rest Bot to it as an admin
- Set the environment variable `TG_REST_CHANNEL_ID` of One Piece Group Bot to the TG Rest Channel
  ID
- Communication with Telegram is handled through [Telethon](https://docs.telethon.dev/), which
  requires a Telegram Client API `api_id`/`api_hash` pair in addition to the bot token. Obtain
  one for free from [my.telegram.org](https://my.telegram.org/apps) (log in with any Telegram
  account, it does not need to be the bot's)
...
- `DASHBOARD_PASSWORD` - Password required to open the dashboard. Change it at any time by
  updating this value and restarting the app - no code changes needed
- `TG_API_ID` - Telegram Client API ID, from [my.telegram.org](https://my.telegram.org/apps)
- `TG_API_HASH` - Telegram Client API Hash, from [my.telegram.org](https://my.telegram.org/apps)

Optionally, `TG_REST_SESSION_STRING` can be set to a Telethon
[StringSession](https://docs.telethon.dev/en/stable/concepts/sessions.html) for the TG Rest Bot.
This is only needed on hosts whose filesystem does not persist between deploys (e.g. Streamlit
Community Cloud); otherwise a local session file is created automatically under `data/`.
